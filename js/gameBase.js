...
function create() {
    ...
    const spaceBetweenCars = config.width / carsPerRow;
    const maxRandomOffset = (spaceBetweenCars - tileSize) / 2;
    for (let j = 0; j < 3; j++){
        for (let i = 0; i < carsPerRow; i++) {
            ...
            carImages[i+j*carsPerRow].setOrigin(0, 0);
    ...
    for (let j = 3; j < 6; j++){
        for (let i = 0; i < carsPerRow; i++) {
            ...
            carImages[i+j*carsPerRow].setOrigin(0, 0);
            carImages[i+j*carsPerRow].angle = 180;
            carImages[i+j*carsPerRow].setVelocity(-carSpeed, 0);
        }
    }
